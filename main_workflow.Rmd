...
### DADA2 Part
```{r}
# Path to all the input fastq files
path <-"input_data/fastq_files_egla"
list.files(path)
...
### Python Part
This code chunk is commented as it does not work as expected for now.
...
This next code chunk, is to be run on command line. Do not run it here in Rstudio.
Command explained:
python3 runs python in the background.
